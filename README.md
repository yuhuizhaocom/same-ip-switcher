...
$Gw         = "10.2.3.254"      # 默认网关（同时作为首选 DNS 自动生效）
$DnsPrimary = "223.5.5.5"       # 备用公网 DNS（第 2 槽位）
...
| `$Gw` | 默认网关 | 改成对应网络的路由器/网关地址；**同时作为首选 DNS**，一般无需另配 DNS |
| `$DnsPrimary` | 备用公网 DNS | 网卡上共 2 个 DNS：`[`$Gw`, `$DnsPrimary`]`。默认 223.5.5.5（阿里），也可改成贵司内部 DNS 或 119.29.29.29 等 |
...
| 切到 WiFi 后：上不了外网、卡顿、或 `169.254.x.x` | 优先确认**是否真连上了 WiFi** 且网线已拔。DNS 由**网关 `$Gw` 自动充当首选**（会议 WiFi 常拦截公网 DNS 但网关解析器可用），脚本会打印 `set DNS on ... = <网关>, <备用>` 供核对。若用 DHCP 临时连一次该 WiFi 能正常上网，则抄下它实际分到的**网关 / 掩码 / DNS** 填进 `network-config.ps1` 改用该网段参数；若 DHCP 也分不到地址，说明该 SSID 需要 802.1x / Web 认证准入 |
| 插回网线后重跑，仍是 WiFi | 脚本会自动**短暂启用有线网卡探测网线**（最长约 7 秒），检测到即切回有线；若始终未检测到，请确认网线另一端插好、且有线网卡没有被系统在其他地方手动禁用。探测无网线时会自动把有线网卡恢复为禁用，不影响结论 |
